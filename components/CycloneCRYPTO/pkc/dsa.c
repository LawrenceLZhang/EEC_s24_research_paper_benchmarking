...
cerror_t dsaWriteSignature(const DsaSignature *signature, uint8_t *data, size_t *length)
{
   cerror_t error;
   size_t k;
   size_t n;
...
cerror_t dsaReadSignature(const uint8_t *data, size_t length, DsaSignature *signature)
{
   cerror_t error;
   Asn1Tag tag;
...
cerror_t dsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
...
cerror_t dsaVerifySignature(const DsaPublicKey *key,
