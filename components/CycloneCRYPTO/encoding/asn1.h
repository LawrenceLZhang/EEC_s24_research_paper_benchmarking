...
cerror_t asn1ReadTag(const uint8_t *data, size_t length, Asn1Tag *tag);
cerror_t asn1ReadSequence(const uint8_t *data, size_t length, Asn1Tag *tag);
cerror_t asn1ReadOctetString(const uint8_t *data, size_t length, Asn1Tag *tag);
cerror_t asn1ReadOid(const uint8_t *data, size_t length, Asn1Tag *tag);

cerror_t asn1ReadBoolean(const uint8_t *data, size_t length, Asn1Tag *tag,
   bool_t *value);

cerror_t asn1ReadInt32(const uint8_t *data, size_t length, Asn1Tag *tag,
   int32_t *value);

cerror_t asn1WriteTag(Asn1Tag *tag, bool_t reverse, uint8_t *data,
   size_t *written);

cerror_t asn1WriteInt32(int32_t value, bool_t reverse, uint8_t *data,
   size_t *written);

#if (MPI_SUPPORT == ENABLED)

cerror_t asn1ReadMpi(const uint8_t *data, size_t length, Asn1Tag *tag,
   Mpi *value);

cerror_t asn1WriteMpi(const Mpi *value, bool_t reverse, uint8_t *data,
   size_t *written);

#endif

cerror_t asn1CheckTag(const Asn1Tag *tag, bool_t constructed, uint_t objClass,
   uint_t objType);

cerror_t asn1CheckOid(const Asn1Tag *tag, const uint8_t *oid, size_t length);

cerror_t asn1DumpObject(const uint8_t *data, size_t length, uint_t level);
