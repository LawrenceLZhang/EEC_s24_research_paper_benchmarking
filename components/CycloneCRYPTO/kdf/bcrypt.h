...
cerror_t bcryptHashPassword(const PrngAlgo *prngAlgo, void *prngContext,
   uint_t cost, const char_t *password, char_t *hash, size_t *hashLen);

cerror_t bcryptVerifyPassword(const char_t *password, const char_t *hash);

cerror_t bcrypt(uint_t cost, const uint8_t *salt, const char_t *password,
   char_t *hash, size_t *hashLen);

cerror_t eksBlowfishSetup(BlowfishContext *context, uint_t cost,
