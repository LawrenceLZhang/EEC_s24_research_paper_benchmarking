...
typedef cerror_t (*HashAlgoCompute)(const void *data, size_t length,
   uint8_t *digest);
...
typedef cerror_t (*CipherAlgoInit)(void *context, const uint8_t *key,
   size_t keyLen);
...
typedef cerror_t (*KemAlgoGenerateKeyPair)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *pk, uint8_t *sk);

typedef cerror_t (*KemAlgoEncapsulate)(const PrngAlgo *prngAlgo,
   void *prngContext, uint8_t *ct, uint8_t *ss, const uint8_t *pk);

typedef cerror_t (*KemAlgoDecapsulate)(uint8_t *ss, const uint8_t *ct,
   const uint8_t *sk);

//Common API for pseudo-random number generators (PRNG)
typedef cerror_t (*PrngAlgoInit)(void *context);

typedef cerror_t (*PrngAlgoSeed)(void *context, const uint8_t *input,
   size_t length);

typedef cerror_t (*PrngAlgoAddEntropy)(void *context, uint_t source,
   const uint8_t *input, size_t length, size_t entropy);

typedef cerror_t (*PrngAlgoRead)(void *context, uint8_t *output, size_t length);
