...
typedef cerror_t (*OcspClientTlsInitCallback)(HttpClientContext *context,
...
cerror_t ocspClientInit(OcspClientContext *context);

#if (OCSP_CLIENT_TLS_SUPPORT == ENABLED)

cerror_t ocspClientRegisterTlsInitCallback(OcspClientContext *context,
   OcspClientTlsInitCallback callback);

#endif

cerror_t ocspClientSetPrng(OcspClientContext *context, const PrngAlgo *prngAlgo,
   void *prngContext);

cerror_t ocspClientSetTimeout(OcspClientContext *context, systime_t timeout);

cerror_t ocspClientSetHost(OcspClientContext *context, const char_t *host);
cerror_t ocspClientSetUri(OcspClientContext *context, const char_t *uri);

cerror_t ocspClientBindToInterface(OcspClientContext *context,
   NetInterface *interface);

cerror_t ocspClientConnect(OcspClientContext *context,
   const IpAddr *serverIpAddr, uint16_t serverPort);

cerror_t ocspClientCreateRequest(OcspClientContext *context,
   const char_t *cert, size_t certLen, const char_t *issuerCert,
   size_t issuerCertLen);

cerror_t ocspClientSendRequest(OcspClientContext *context);

cerror_t ocspClientValidateResponse(OcspClientContext *context,
   const char_t *cert, size_t certLen, const char_t *issuerCert,
   size_t issuerCertLen);
...
cerror_t ocspClientDisconnect(OcspClientContext *context);
cerror_t ocspClientClose(OcspClientContext *context);
