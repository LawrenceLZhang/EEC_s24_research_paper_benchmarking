...
void perfDESCipher(DesContext *context, const TestVector *test)
{
    ...
    desInit(context, test->key, sizeof(test->key));
    ...
    for (count = 0; count < TEST_ITERATIONS; ++count) {
        for (int blocks = 0; blocks < BUFFER_SIZE; blocks += BLOCK_BYTES) {
            desEncryptBlock(context, buffer + blocks, buffer + blocks);
        }
    }
    ...
    for (count = 0; count < TEST_ITERATIONS; ++count) {
        for (int blocks = 0; blocks < BUFFER_SIZE; blocks += BLOCK_BYTES) {
            desDecryptBlock(context, buffer + blocks, buffer + blocks);
        }
    }
    elapsed = esp_timer_get_time() - start;
    printf("%f us/B, ", elapsed / (TEST_ITERATIONS * BUFFER_SIZE));
...
void testDES()
{
    perfDESCipher(&des, &testVectorDES);
}
